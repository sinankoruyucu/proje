...
class CustomerManagementGUI:
    ...
    def open_settings(self):
        ...
        settings_dialog.geometry(f"600x700+{x}+{y}")
        ...
        def delete_all_data():
            ...
            if password == self.settings_password:
                confirm = messagebox.askyesno("Onay", "Tüm veriler silinecek!\nEmin misiniz?")
                if confirm:
        ...
        delete_btn = tk.Button(button_container, text="🗑️ Verileri Sil", command=delete_all_data,
        ...
        def on_delete_enter(e):
            delete_btn.configure(bg='#c82333')
        def on_delete_leave(e):
            delete_btn.configure(bg='#dc3545')
        def on_cancel_enter(e):
            cancel_btn.configure(bg='#545b62')
        def on_cancel_leave(e):
            cancel_btn.configure(bg='#6c757d')
    ...
    def complete_overdue_orders(self):
        ...
        try:
            ...
            # Modern scrollable container (DANIŞAN KAYIT GİBİ)
            container_frame = tk.Frame(confirm_dialog, bg='#f8f9fa')
            container_frame.pack(fill=tk.BOTH, expand=True, padx=20, pady=20)
            
            # Canvas for scrolling
            canvas = tk.Canvas(container_frame, bg='#f8f9fa', highlightthickness=0, bd=0)
            
            # Modern custom scrollbar frame
            scrollbar_frame = tk.Frame(container_frame, bg='#f8f9fa', width=8)
            scrollbar_bg = tk.Canvas(scrollbar_frame, bg='#f8f9fa', width=8, highlightthickness=0, bd=0)
            ...
            # Ana container - Modern card tasarımı (scrollable_frame içinde)
            main_container = tk.Frame(scrollable_frame, bg='#f8f9fa')
            ...
            # Bind events
            canvas.bind("<MouseWheel>", smooth_scroll)
            scrollbar_bg.bind("<Button-1>", on_scrollbar_click)
            scrollbar_bg.bind("<B1-Motion>", on_scrollbar_drag)
            scrollbar_bg.bind("<ButtonRelease-1>", on_scrollbar_release)
            ...
            # Pack widgets
            canvas.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
            scrollbar_frame.pack(side=tk.RIGHT, fill=tk.Y, padx=0)
            scrollbar_frame.pack_propagate(False)
            scrollbar_bg.pack(fill="both", expand=True)
            ...
            # TABLO DOĞRUDAN ANA CONTAINER İÇİNDE
            list_card = tk.Frame(main_container, bg='white', relief='flat', bd=0)
            ...
            tree_frame = tk.Frame(list_container, bg='white')
            tree_frame.pack(fill=tk.BOTH, expand=True)
            
            # Modern Treeview
            columns = ('Status', 'Müşteri', 'Ürün', 'Bitiş Tarihi', 'Gecikme', 'Durum')
            tree = ttk.Treeview(tree_frame, columns=columns, show='headings', height=15, selectmode='extended', style='Modern.Treeview')
            
            # BASİT SCROLLBAR - DANIŞAN KAYIT GİBİ
            scrollbar = ttk.Scrollbar(tree_frame, orient=tk.VERTICAL, command=tree.yview, style='Modern.Vertical.TScrollbar')
            ...
            # PACK TREE VE SCROLLBAR - EN ÖNEMLİSİ BU!
            ...
            # MOUSE WHEEL İÇİN CANVAS VE TÜM WİDGET'LARA BIND ET
            canvas.bind("<MouseWheel>", smooth_scroll)
            confirm_dialog.bind("<MouseWheel>", smooth_scroll)
            main_container.bind("<MouseWheel>", smooth_scroll)
            list_card.bind("<MouseWheel>", smooth_scroll)
            tree.bind("<MouseWheel>", smooth_scroll)
            
            
            # Initialize scroll after everything is packed
            confirm_dialog.after(100, lambda: canvas.configure(scrollregion=canvas.bbox("all")))
